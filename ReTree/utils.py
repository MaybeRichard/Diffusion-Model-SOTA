...
class DDPMDataset_sep(Dataset):
    def __init__(self, data_paths, label_paths, cls_path, img_size):
        self.data_paths = data_paths
        self.label_paths = label_paths
        self.cls_lis = self.read_csv(cls_path)
    ...
    def __getitem__(self, index):
        MA, EX, HE, SE = Image.open(self.data_paths[index]).convert('L'), Image.open(self.data_paths[index].replace('MA', 'EX')).convert('L'), \
                                Image.open(self.data_paths[index].replace('MA', 'HE')).convert('L'), Image.open(self.data_paths[index].replace('MA', 'SE')).convert('L')
        # VE = Image.open(self.data_paths[index].replace('MA', 'VE')).convert('L')

        #     MA, EX, HE, SE = self.transform2(MA), self.transform2(EX), self.transform2(HE), self.transform2(SE)

        lb = Image.open(self.label_paths[index]).convert('L')
        # lb = Image.fromarray(np.uint8(np.where(np.array(lb)>=127, 255, 0)))
        cls = int(self.cls_lis[self.data_paths[index].split('/')[-1]])

        if self.transform2:
            MA, EX, HE, SE = self.transform2(MA), self.transform2(EX), self.transform2(HE), self.transform2(SE)
            # VE = self.transform2(VE)
            lb = self.transform2(lb)

        if random.random() < 0.5:
            MA, EX, HE, SE, lb = TF.hflip(MA), TF.hflip(EX), TF.hflip(HE), TF.hflip(SE), TF.hflip(lb)
            # VE = TF.hflip(VE)
        if random.random() > 0.5:
            MA, EX, HE, SE, lb = TF.vflip(MA), TF.vflip(EX), TF.vflip(HE), TF.vflip(SE), TF.vflip(lb)
            # VE = TF.vflip(VE)
        # img = torch.cat([MA, EX, HE, SE, VE], dim=0)
        img = torch.cat([MA, EX, HE, SE], dim=0)
...
class DDPMDataset_csv_generate(Dataset):
    def __init__(self, label_paths, cls_path, img_size):
        self.label_paths = label_paths
        self.cls_lis = self.read_csv(cls_path)
    ...
    def __getitem__(self, index):
        info = self.cls_lis[index]
        name = info[0]
        cls = int(info[1])
        lb = Image.open(os.path.join(self.label_paths, name)).convert('L')
        ...
        return lb, cls, name, index

    def __len__(self):
        return len(self.cls_lis)
class DDPMDataset(Dataset):
    def __init__(self, data_paths, label_paths, ma_paths, img_size, outname=False, augment=True):
        self.data_paths = data_paths
        self.label_paths = label_paths
        self.ma_paths = ma_paths
    ...
    def __getitem__(self, index):
        ...
        lb = Image.open(self.label_paths[index]).convert('L')
        ma = Image.open(self.ma_paths[index]).convert('L')
        he = Image.open(self.ma_paths[index].replace('MA', 'HE')).convert('L')
        se = Image.open(self.ma_paths[index].replace('MA', 'SE')).convert('L')
        ex = Image.open(self.ma_paths[index].replace('MA', 'EX')).convert('L')
        fused = Image.open(self.ma_paths[index].replace('MA', 'lesion_all')).convert('L')
        # lb = Image.fromarray(np.uint8(np.where(np.array(lb)>=127, 255, 0)))
        ...
        if self.transform2:
            lb = self.transform2(lb)
            ma, he, se, ex = self.transform2(ma), self.transform2(he), self.transform2(se), self.transform2(ex)
            fused = self.transform2(fused)

            lb[fused==0] = 0
        if self.augment:
            if random.random() < 0.5:
                img, lb, ma, he, se, ex = TF.hflip(img), TF.hflip(lb), TF.hflip(ma), TF.hflip(he), TF.hflip(se), TF.hflip(ex)
                fused = TF.hflip(fused)
            if random.random() > 0.5:
                img, lb, ma, he, se, ex = TF.vflip(img), TF.vflip(lb), TF.vflip(ma), TF.vflip(he), TF.vflip(se), TF.vflip(ex)
                fused = TF.vflip(fused)
        else:
            pass

        if self.outname:
            return img, lb, fused, ma, he, se, ex, self.data_paths[index].split('/')[-1]
        else:
            return img, lb, fused, ma, he, se, ex
