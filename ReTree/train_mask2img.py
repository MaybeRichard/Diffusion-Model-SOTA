"""
Mask-guided Image Generation using DDPM
用法: python train_mask2img.py --data_root /path/to/dataset --device cuda:0
...
class Diffusion:
    ...
    def sample(self, model, mask, n=1):
        ...
        for i in tqdm(reversed(range(1, self.noise_steps)), desc="Sampling", leave=False):
            ...
            # 预测噪声
            predicted_noise = model(x, t, mask=mask)
...
def train(args):
    ...
    # 模型: 输入通道 = 3(RGB) + 1(mask) = 4, 输出通道 = 3(RGB)
    model = DDPM(
        img_channels=4,      # 输入: RGB + mask
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for i, (images, masks) in enumerate(pbar):
            ...
            # 模型预测噪声 (条件: mask)
            predicted_noise = model(x=x_t, t=t, mask=masks)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Mask-guided DDPM Image Generation")
    ...
    parser.add_argument('--data_root', type=str, required=True,
                        help='数据集根目录 (包含 image/ 和 mask/ 子目录)')
    ...
    # 保存相关
    parser.add_argument('--output_dir', type=str, default='./outputs/mask2img',
                        help='输出目录')
    parser.add_argument('--sample_interval', type=int, default=500, help='每N个batch生成样本(设大些避免拖慢训练)')
    ...
    if args.mode == 'train':
        ...
        if args.mask_dir is None:
            args.mask_dir = os.path.join(args.data_root, "mask")
        generate(args)
